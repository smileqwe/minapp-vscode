...
const identifierCollector_1 = require("../../plugin/lib/identifierCollector");
...
describe('identifierCollector — atomic fns', () => {
    ...
    describe('collectObjectLiteralKeys (T2.1)', () => {
        it('T2.1.1: 平铺对象字面量命中 foo', () => {
            ...
            const hits = (0, identifierCollector_1.collectObjectLiteralKeys)(source, 'prop', byName('userName'));
        ...
        it('T2.1.2: 深层对象字面量（被 factory 包裹）也能命中', () => {
            const { source } = loadFixture('factory-wrap.js');
            const hits = (0, identifierCollector_1.collectObjectLiteralKeys)(source, 'prop', byName('age'));
            assert.strictEqual(hits.length, 1);
            assert.strictEqual(hits[0].source, 'object-literal');
        });
        it('T2.1.5: method 查询命中对象字面量中的方法声明 go()', () => {
            const { source } = loadFixture('native-page.js');
            const hits = (0, identifierCollector_1.collectObjectLiteralKeys)(source, 'method', byName('go'));
        ...
        it('T2.1.4: computed key 不应命中', () => {
            ...
            const hits = (0, identifierCollector_1.collectObjectLiteralKeys)(sf, 'prop', byName('key'));
    ...
    describe('collectClassMembers (T2.2)', () => {
        it('T2.2.1: class 方法声明命中 onTap', () => {
            ...
            const hits = (0, identifierCollector_1.collectClassMembers)(source, 'method', byName('onTap'));
            assert.strictEqual(hits.length, 1);
            assert.strictEqual(hits[0].source, 'class-member');
        });
        it('T2.2.2: class 属性声明命中 data', () => {
            const { source } = loadFixture('decorator-class.ts');
            const hits = (0, identifierCollector_1.collectClassMembers)(source, 'prop', byName('data'));
            assert.strictEqual(hits.length, 1);
            assert.strictEqual(hits[0].source, 'class-member');
        });
        it('T2.2.3: private 字段 #privateField 不被命中', () => {
            const { source } = loadFixture('class-extends.js');
            const hits = (0, identifierCollector_1.collectClassMembers)(source, 'prop', byName('privateField'));
            assert.strictEqual(hits.length, 0);
        });
        it('T2.2.4: static 属性 foo 能命中', () => {
            const { source } = loadFixture('decorator-class.ts');
            const hits = (0, identifierCollector_1.collectClassMembers)(source, 'prop', byName('foo'));
            assert.strictEqual(hits.length, 1);
        });
    });
    /* ------------------------------------------------------------------ 2.3 */
    describe('collectSetDataKeys (T2.3)', () => {
        it('T2.3.1: this.setData({ loading }) 命中 loading', () => {
            ...
            const hits = (0, identifierCollector_1.collectSetDataKeys)(source, byName('loading'));
            assert.strictEqual(hits.length, 1);
            assert.strictEqual(hits[0].source, 'setData');
        });
        it('T2.3.2: 路径 key "list[0].name" 取首段 list', () => {
            const { source } = loadFixture('set-data.js');
            const hits = (0, identifierCollector_1.collectSetDataKeys)(source, byName('list'));
            assert.strictEqual(hits.length, 1);
        });
        it('T2.3.3: that.setData 也支持', () => {
            const { source } = loadFixture('set-data.js');
            const hits = (0, identifierCollector_1.collectSetDataKeys)(source, byName('foo'));
            assert.strictEqual(hits.length, 1);
        });
        it('T2.3.4: setData 参数是变量时不收集', () => {
            const { source } = loadFixture('set-data.js');
            // bar 只存在于 dynamicObj 里，不应被 setData 路径收集
            const hits = (0, identifierCollector_1.collectSetDataKeys)(source, byName('bar'));
            assert.strictEqual(hits.length, 0);
        });
    });
    /* ------------------------------------------------------------------ 2.4 */
    describe('collectReturnObjectKeys (T2.4)', () => {
        it('T2.4.1: shorthand return 指向 const 定义位置', () => {
            ...
            const hits = (0, identifierCollector_1.collectReturnObjectKeys)(source, 'prop', byName('count'));
            assert.strictEqual(hits.length, 1);
        ...
        it('T2.4.3: 箭头函数直接返回对象 buildState', () => {
            const { source } = loadFixture('composition-api.js');
            const hits = (0, identifierCollector_1.collectReturnObjectKeys)(source, 'prop', byName('foo'));
            assert.strictEqual(hits.length, 1);
            assert.strictEqual(hits[0].source, 'return-destructure');
        });
        it('T2.4.4: method 查询命中 return 对象中的 handleTap', () => {
            const { source } = loadFixture('composition-api.js');
            const hits = (0, identifierCollector_1.collectReturnObjectKeys)(source, 'method', byName('handleTap'));
    ...
    describe('collectAssignmentKeys (T2.5)', () => {
        it('T2.5.1: this.userName 命中', () => {
            ...
            const hits = (0, identifierCollector_1.collectAssignmentKeys)(source, 'prop', byName('userName'));
            assert.strictEqual(hits.length, 1);
            assert.strictEqual(hits[0].source, 'assignment');
        });
        it('T2.5.2: Home.prototype.onTap 命中', () => {
            const { source } = loadFixture('this-assign.js');
            const hits = (0, identifierCollector_1.collectAssignmentKeys)(source, 'method', byName('onTap'));
            assert.strictEqual(hits.length, 1);
        });
        it('T2.5.3: globalThis.foo 不命中', () => {
            const { source } = loadFixture('this-assign.js');
            const hits = (0, identifierCollector_1.collectAssignmentKeys)(source, 'prop', byName('foo'));
            assert.strictEqual(hits.length, 0);
        });
    });
    /* ------------------------------------------------------------------ 2.6 */
    describe('collectSpreadKeys (T2.6)', () => {
        it('T2.6.1: ...base 解析到文件顶层 const base 并命中 foo', () => {
            ...
            const hits = (0, identifierCollector_1.collectSpreadKeys)(source, 'prop', byName('foo'));
        ...
        it('T2.6.2: Object.assign({}, mixinA) 展开 mixinA.bar', () => {
            const { source } = loadFixture('spread-in-file.js');
            const hits = (0, identifierCollector_1.collectSpreadKeys)(source, 'prop', byName('bar'));
...
describe('identifierCollector — collectAllIdentifiers (T1)', () => {
    it('T1.1: 未知 factory 包裹的 data.userName 可被找到', () => {
        const { source } = loadFixture('factory-wrap.js');
        const hits = (0, identifierCollector_1.collectAllIdentifiers)(source, 'prop', byName('userName'));
        assert.ok(hits.some(h => h.name === 'userName'));
    });
    it('T1.2: 装饰器+class property 中的 age 可被找到', () => {
        const { source } = loadFixture('decorator-class.ts');
        const hits = (0, identifierCollector_1.collectAllIdentifiers)(source, 'prop', byName('age'));
    ...
    it('T1.3: class 方法 onTap 能通过 class-member 路径命中', () => {
        const { source } = loadFixture('class-extends.js');
        const hits = (0, identifierCollector_1.collectAllIdentifiers)(source, 'method', byName('onTap'));
        assert.ok(hits.some(h => h.source === 'class-member'));
    });
    it('T1.4: Composition API 中 return { count } 能被找到', () => {
        const { source } = loadFixture('composition-api.js');
        const hits = (0, identifierCollector_1.collectAllIdentifiers)(source, 'prop', byName('count'));
        assert.ok(hits.some(h => h.source === 'return-destructure'));
    });
    it('T1.5: 原生 Page 的 methods.go 能被找到', () => {
        const { source } = loadFixture('native-page.js');
        const hits = (0, identifierCollector_1.collectAllIdentifiers)(source, 'method', byName('go'));
        assert.ok(hits.length >= 1);
    });
    it('T1.6: 不存在的 prop 返回空', () => {
        const { source } = loadFixture('native-page.js');
        const hits = (0, identifierCollector_1.collectAllIdentifiers)(source, 'prop', byName('doesNotExist'));
        assert.strictEqual(hits.length, 0);
    });
    it('T1.7: 兜底收集的 hit 置信度应为 low', () => {
        const { source } = loadFixture('native-page.js');
        const hits = (0, identifierCollector_1.collectAllIdentifiers)(source, 'prop', byName('foo'));
        assert.ok(hits.length >= 1);
...
describe('rankAndDedupe (T4)', () => {
    function hit(start, end, source, name = 'x') {
        return { start, end, name, detail: name, source, confidence: 'low' };
    }
    it('T4.1: 同位置多 source 只保留优先级最高(class-member > object-literal)', () => {
        const input = [hit(10, 11, 'object-literal'), hit(10, 11, 'class-member'), hit(10, 11, 'setData')];
        const out = (0, identifierCollector_1.rankAndDedupe)(input);
    ...
    it('T4.2: 不同位置同名命中全部保留,按优先级排序', () => {
        const input = [hit(30, 31, 'setData'), hit(10, 11, 'spread'), hit(20, 21, 'object-literal')];
        const out = (0, identifierCollector_1.rankAndDedupe)(input);
        assert.strictEqual(out.length, 3);
        assert.deepStrictEqual(out.map(h => h.source), ['object-literal', 'setData', 'spread'], '应按 class > object > return > setData > assign > spread 排序');
    });
    it('T4.3: 同优先级不同位置按 start 升序', () => {
        ...
        const out = (0, identifierCollector_1.rankAndDedupe)(input);
        assert.deepStrictEqual(out.map(h => h.start), [10, 30, 50]);
    });
    it('T4.4: 空输入返回空数组', () => {
        assert.deepStrictEqual((0, identifierCollector_1.rankAndDedupe)([]), []);
    });
...
//# sourceMappingURL=identifierCollector.test.js.map