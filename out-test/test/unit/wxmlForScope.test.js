...
const wxmlForScope_1 = require("../../plugin/lib/wxmlForScope");
...
describe('wxmlForScope', () => {
    it('W1: 默认 item/index 都可见', () => {
        const { text, offset } = mark('<view wx:for="{{list}}"><text>{{|item}}</text></view>');
        const names = (0, wxmlForScope_1.getVisibleWxForBindings)(text, offset).map(b => b.name);
        assert.ok(names.includes('item'));
        assert.ok(names.includes('index'));
    });
    it('W2: wx:for-item 重命名后 item 不再可见,新名生效', () => {
        const { text, offset } = mark('<view wx:for="{{list}}" wx:for-item="user">{{|user.name}}</view>');
        const names = (0, wxmlForScope_1.getVisibleWxForBindings)(text, offset).map(b => b.name);
    ...
    it('W3: wx:for-index 重命名', () => {
        const { text, offset } = mark('<view wx:for="{{list}}" wx:for-index="idx">{{|idx}}</view>');
        const names = (0, wxmlForScope_1.getVisibleWxForBindings)(text, offset).map(b => b.name);
    ...
    it('W4: wx:for-items 作为 wx:for 的别名', () => {
        const { text, offset } = mark('<view wx:for-items="{{list}}">{{|item}}</view>');
        const bindings = (0, wxmlForScope_1.getVisibleWxForBindings)(text, offset);
        assert.ok(bindings.some(b => b.name === 'item' && b.sourceExpr === 'list'));
    });
    it('W5: 嵌套 for,内层 wx:for-item="tag",index 被外层 idx 覆盖', () => {
        const { text, offset } = mark('<view wx:for="{{list}}" wx:for-item="user" wx:for-index="idx">' +
            '<text wx:for="{{user.tags}}" wx:for-item="tag">{{|tag}}-{{idx}}</text>' +
            '</view>');
        const all = (0, wxmlForScope_1.getVisibleWxForBindings)(text, offset);
        const uniq = (0, wxmlForScope_1.dedupeByName)(all).map(b => b.name);
    ...
    it('W6: 内层覆盖外层同名(内层 item 胜出)', () => {
        const { text, offset } = mark('<view wx:for="{{outer}}">' +
        ...
        // dedupeByName 只保留内层
        const uniq = (0, wxmlForScope_1.dedupeByName)(all);
        const uniqItem = uniq.find(b => b.name === 'item');
        assert.strictEqual(uniqItem.sourceExpr, 'inner');
    });
    it('W7: block 标签 wx:for 也能解析', () => {
        const { text, offset } = mark('<block wx:for="{{list}}"><view>{{|item}}</view></block>');
        const names = (0, wxmlForScope_1.getVisibleWxForBindings)(text, offset).map(b => b.name);
        assert.ok(names.includes('item'));
    });
    it('W8: 自闭合标签的 wx:for 作用域在标签结束后失效', () => {
        // 自闭合元素通常没有子节点,这里测光标落在元素之后应不可见
        const { text, offset } = mark('<view wx:for="{{list}}" />{{|item}}');
        const names = (0, wxmlForScope_1.getVisibleWxForBindings)(text, offset).map(b => b.name);
        assert.ok(!names.includes('item'), '自闭合元素后应不可见');
    });
    it('W9: 光标在 wx:for 开标签之前不命中', () => {
        const { text, offset } = mark('{{|item}}<view wx:for="{{list}}"><text>x</text></view>');
        const names = (0, wxmlForScope_1.getVisibleWxForBindings)(text, offset).map(b => b.name);
        assert.strictEqual(names.length, 0);
    });
    it('W10: 光标在闭合标签之后不命中', () => {
        const { text, offset } = mark('<view wx:for="{{list}}"><text>x</text></view>{{|item}}');
        const names = (0, wxmlForScope_1.getVisibleWxForBindings)(text, offset).map(b => b.name);
        assert.strictEqual(names.length, 0);
    });
    it('W11: 注释中的 wx:for 被忽略', () => {
        const { text, offset } = mark('<!-- <view wx:for="{{list}}"> --><view>{{|item}}</view>');
        const names = (0, wxmlForScope_1.getVisibleWxForBindings)(text, offset).map(b => b.name);
        assert.strictEqual(names.length, 0);
    });
    it('W12: 属性值中的 `>` 不会截断标签解析', () => {
        const { text, offset } = mark('<view wx:for="{{list}}" title="a>b"><text>{{|item}}</text></view>');
        const names = (0, wxmlForScope_1.getVisibleWxForBindings)(text, offset).map(b => b.name);
        assert.ok(names.includes('item'));
    });
    it('W13: defOffset 正确指向属性名起点', () => {
        const wxml = '<view wx:for="{{list}}" wx:for-item="user">{{user.name}}</view>';
        const all = (0, wxmlForScope_1.collectWxForBindings)(wxml);
...
//# sourceMappingURL=wxmlForScope.test.js.map