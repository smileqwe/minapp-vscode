...
function getPropNameText(
  ...
  if (ts.isPrivateIdentifier(n)) return undefined // 跳过 #private
  if (ts.isComputedPropertyName(n)) return undefined // 动态 key 跳过
  return undefined
}
...
export function collectObjectLiteralKeys(
  sourceFile: ts.SourceFile,
  type: QueryType,
  matchesProp: (name: string) => boolean
): CollectedHit[] {
...
export function collectSetDataKeys(sourceFile: ts.SourceFile, matchesProp: (name: string) => boolean): CollectedHit[] {
  const hits: CollectedHit[] = []
  ...
  function visit(node: ts.Node) {
    if (
      ...
      const isThisLike =
        receiver.kind === ts.SyntaxKind.ThisKeyword || (ts.isIdentifier(receiver) && THIS_LIKE_NAMES.has(receiverText))
...
export function collectReturnObjectKeys(
  sourceFile: ts.SourceFile,
  type: QueryType,
  matchesProp: (name: string) => boolean
): CollectedHit[] {
  ...
  function findReturnedObjects(fn: ts.FunctionLikeDeclarationBase): ts.ObjectLiteralExpression[] {
    ...
    if (ts.isBlock(body)) {
      body.forEachChild(function walk(n) {
        if (ts.isReturnStatement(n) && n.expression) {
          ...
          if (ts.isObjectLiteralExpression(e)) objs.push(e)
        } else {
          n.forEachChild(walk)
        }
      })
  ...
  function findLocalDecl(fn: ts.FunctionLikeDeclarationBase, name: string): ts.Node | undefined {
    const body = fn.body
    ...
    for (const stmt of body.statements) {
      ...
      for (const decl of stmt.declarationList.declarations) {
        if (ts.isIdentifier(decl.name) && decl.name.text === name) return decl.name
        if (ts.isObjectBindingPattern(decl.name)) {
          for (const el of decl.name.elements) {
            if (ts.isIdentifier(el.name) && el.name.text === name) return el.name
          }
        }
...
export function collectAssignmentKeys(
  sourceFile: ts.SourceFile,
  type: QueryType,
  matchesProp: (name: string) => boolean
): CollectedHit[] {
  ...
  function visit(node: ts.Node) {
    if (
      ...
      let accepted = false
      if (receiver.kind === ts.SyntaxKind.ThisKeyword) accepted = true
      else if (ts.isIdentifier(receiver) && THIS_LIKE_NAMES.has(receiver.text)) accepted = true
      else if (
...
export function collectSpreadKeys(
  sourceFile: ts.SourceFile,
  type: QueryType,
  matchesProp: (name: string) => boolean
): CollectedHit[] {
  ...
  function collectFromObjectLike(expr: ts.Expression, visited: Set<string>): void {
  ...
  function resolveAndCollect(expr: ts.Expression, visited: Set<string>): void {
    if (ts.isIdentifier(expr)) {
      const name = expr.text
      if (visited.has(name)) return
      visited.add(name)
      const init = topLevelBindings.get(name)
      if (init) collectFromObjectLike(init, visited)
      return
    }
...
const SOURCE_PRIORITY: Record<CollectSource, number> = {
  'class-member': 1,
  'object-literal': 2,
 ...
 */
export function rankAndDedupe(hits: CollectedHit[]): CollectedHit[] {
  const byKey = new Map<string, CollectedHit>()
  for (const h of hits) {
